...
public class ChinaVerificationCodeTst extends TestCase {
	...
	private static final Logger cat = Logger.getLogger(
			ChinaVerificationCodeTst.class.getName(), EPPCatFactory
	...
	public void testNormalRegistrationOperations() {
		...
		try {
			...
			EPPNameVerification theVsp = new EPPNameVerification(theVspSession);

			System.out
					.println("testNormalRegistrationOperations: Got sessions to the Registry and to the Vsp");
			...
			System.out
					.println("testNormalRegistrationOperations: Generated unique domain label ("
							+ theLabel
							+ ") and domain name ("
							+ theDomainName
							+ ") for flow");
			...
			try {
				...
				if (theResults.size() != 1) {
					Assert.fail("Domain availability check result of 1 domain expected, but received "
							+ theResults.size());
				}
				...
				System.out
						.println("testNormalRegistrationOperations: Step 1 - Domain name "
			...
			try {
				...
				EPPNameVerificationCheckResp theResponse = theVsp.sendCheck();

				List<EPPNameVerificationCheckResult> theResults = theResponse
						.getCheckResults();

				if (theResults.size() != 1) {
					Assert.fail("Vsp label check result of 1 label expected, but received "
							+ theResults.size());
				}
				...
				System.out
						.println("testNormalRegistrationOperations: Step 2 - Label "
								+ theLabel
			...
			try {
				...
				EPPNameVerificationCreateResp theResponse = theVsp.sendCreate();

				if (!(theResponse.getCreateResult() instanceof EPPNameVerificationCreateSuccess)) {
					Assert.fail("DNVC create was not successful with result = "
				...
				System.out
						.println("testNormalRegistrationOperations: Step 3 - DNVC successfully created, type = "
			...
			try {
				...
				System.out
						.println("testNormalRegistrationOperations: Step 4 - Domain name "
								+ theDomainName
			...
			try {
				...
				if (!(theResponse.getCreateResult() instanceof EPPNameVerificationCreateSuccess)) {
					Assert.fail("RNVC create was not successful with result = "
							+ theResponse.getCreateResult());
				}
				...
				System.out
						.println("testNormalRegistrationOperations: Step 5 - RNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				System.out
						.println("testNormalRegistrationOperations: Step 6 - Domain name "
								+ theDomainName
			...
			try {
				...
				Assert.assertTrue(theVerificationExt.getStatus() == EPPVerificationCodeInfData.Status.COMPLIANT);

				System.out
						.println("testNormalRegistrationOperations: Domain name "
			...
			if (ex.getSession() == theRegistrySession) {
				...
				Assert.fail("InvalidateSessionException received with no matching session");
			}
		}
		finally {
	...
	public void testUpfrontVerification() {
		...
		try {
			...
			System.out
					.println("testUpfrontVerification: Got sessions to the Registry and to the Vsp");
			...
			System.out
					.println("testUpfrontVerification: Generated unique domain label ("
							+ theLabel
							+ ") and domain name ("
							+ theDomainName
							+ ") for flow");
			...
			try {
				...
				System.out
						.println("testUpfrontVerification: Step 1 - Domain name "
								+ theDomainName + " is available");
			...
			try {
				...
				System.out.println("testUpfrontVerification: Step 2 - Label "
						+ theLabel
						+ " is available (not restricted or prohibited)");
			}
			catch (Exception ex) {
			...
			try {
				...
				theDnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testUpfrontVerification: Step 3 - DNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				theRnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testUpfrontVerification: Step 4 - RNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				System.out
						.println("testUpfrontVerification: Step 5 - Domain name "
								+ theDomainName
								+ " successfully created with DNVC (type = "
								+ theDnvc.getCodeType()
								+ ", code = "
	...
	public void testExpedientRegistration() {
		...
		try {
			...
			System.out
					.println("testExpedientRegistration: Got sessions to the Registry and to the Vsp");
			...
			System.out
					.println("testExpedientRegistration: Generated unique domain label ("
							+ theLabel
							+ ") and domain name ("
							+ theDomainName
							+ ") for flow");
			...
			try {
				...
				System.out
						.println("testExpedientRegistration: Step 1 - Domain name "
								+ theDomainName + " is available");
			...
			try {
				...
				System.out
						.println("testExpedientRegistration: Step 2 - Domain name "
								+ theDomainName
			...
			try {
				...
				System.out.println("testExpedientRegistration: Step 3 - Label "
						+ theLabel
						+ " is available (not restricted or prohibited)");
			}
			catch (Exception ex) {
			...
			try {
				...
				theDnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testExpedientRegistration: Step 4 - DNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				theRnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testExpedientRegistration: Step 5 - RNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				System.out
						.println("testExpedientRegistration: Step 6 - Domain name "
								+ theDomainName
	...
	public void testDelayedRealNameVerification() {
		...
		try {
			...
			System.out
					.println("testDelayedRealNameVerification: Got sessions to the Registry and to the Vsp");
			...
			if (theChangePollMsg == null) {
				...
				System.out
						.println("testDelayedRealNameVerification: Generated unique domain label ("
								+ theLabel
								+ ") and domain name ("
				...
				try {
					...
					System.out
							.println("testDelayedRealNameVerification: Step 1 - Domain name "
									+ theDomainName + " is available");
				...
				try {
					...
					System.out
							.println("testDelayedRealNameVerification: Step 2 - Label "
									+ theLabel
									+ " is available (not restricted or prohibited)");
				}
				catch (Exception ex) {
				...
				try {
					...
					theDnvc = theSuccessResult.getSignedCode();
					System.out
							.println("testDelayedRealNameVerification: Step 3 - DNVC successfully created, type = "
									+ theSuccessResult.getCodeType()
									+ ", code  = " + theSuccessResult.getCode());
				}
				catch (Exception ex) {
					TestUtil.handleException(theVspSession, ex);
				}
				...
				try {
					...
					System.out
							.println("testDelayedRealNameVerification: Step 4 - Domain name "
									+ theDomainName
									+ " successfully created with DNVC (type = "
									+ theDnvc.getCodeType()
									+ ", code = "
				...
				try {
					...
					theDomain
							.addExtension(new EPPVerificationCodeInfo());
					...
					Assert.assertTrue(theVerificationExt.getStatus() == EPPVerificationCodeInfData.Status.PENDING_COMPLIANCE);

					// Assert there is a profile set
					Assert.assertTrue(theVerificationExt.hasProfile());

					// Assert there are missing codes
					Assert.assertTrue(theVerificationExt.getProfile()
							.hasMissingCodes());

					// Assert the missing code is a real-name code
					Assert.assertTrue(theVerificationExt.getProfile()
							.getMissingCodes().get(0).getType()
							.equals("real-name"));

					// Get due date for RNVC

					System.out
							.println("testDelayedRealNameVerification: Domain name "
									+ theDomainName
				...
				System.out
						.println("testDelayedRealNameVerification: Step 5 - Got Change Poll Message for domain "
								+ theDomainName);
				...
				try {
					...
					theRnvc = theSuccessResult.getSignedCode();
					System.out
							.println("testDelayedRealNameVerification: Step 6 - RNVC successfully created, type = "
									+ theSuccessResult.getCodeType()
									+ ", code  = " + theSuccessResult.getCode());
				}
				catch (Exception ex) {
				...
				try {
					...
					System.out
							.println("testDelayedRealNameVerification: Step 7 - Domain name "
									+ theDomainName
									+ " successfully update with RNVC (type = "
									+ theRnvc.getCodeType()
									+ ", code = "
	...
	public void testDelayedDomainVerification() {
		...
		try {
			...
			System.out
					.println("testDelayedDomainVerification: Got sessions to the Registry and to the Vsp");

			// --- Generate unique domain label and domain name

			String theDomainName = this.makeDomainName(
					"cvct-delayed-domain-ver-", ".com");
			System.out.println("theDomainName = " + theDomainName);
			String theLabels[] = theDomainName.split("\\.");
			String theLabel = theLabels[0];
			String theTld = theLabels[1];
			...
			System.out
					.println("testDelayedDomainVerification: Generated unique domain label ("
							+ theLabel
							+ ") and domain name ("
							+ theDomainName
							+ ") for flow");
			...
			try {
				...
				System.out
						.println("testDelayedDomainVerification: Step 1 - Domain name "
								+ theDomainName + " is available");
			...
			try {
				...
				theRnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testDelayedDomainVerification: Step 2 - RNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				System.out
						.println("testDelayedDomainVerification: Step 3 - Domain name "
								+ theDomainName
			...
			try {
				...
				System.out
						.println("testDelayedDomainVerification: Step 4 - Label "
								+ theLabel
								+ " is available (not restricted or prohibited)");
			}
			catch (Exception ex) {
			...
			try {
				...
				theDnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testDelayedDomainVerification: Step 5 - DNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				System.out
						.println("testDelayedDomainVerification: Step 6 - Domain name "
								+ theDomainName
								+ " successfully update with DNVC (type = "
								+ theDnvc.getCodeType()
								+ ", code = "
			...
			try {
				...
				System.out
						.println("testDelayedDomainVerification: Domain name "
								+ theDomainName + " verification is compliant");
			}
			catch (Exception ex) {
				TestUtil.handleException(theRegistrySession, ex);
	...
	public void testTransferOfCompliantDomain() {
		...
		if (!this.sessionPool.hasSystemSessionPool(POOL_REGISTRY_2)) {
			System.out
					.println("testTransferOfCompliantDomain: "
							+ POOL_REGISTRY_2
		...
		try {
			...
			System.out
					.println("testTransferOfCompliantDomain: Got sessions to the Registry (1 and 2) and to the Vsp");

			// --- Generate unique domain label and domain name

			String theDomainName = this.makeDomainName(
					"cvct-transfer-compliant-", ".com");
			System.out.println("theDomainName = " + theDomainName);
			String theLabels[] = theDomainName.split("\\.");
			String theLabel = theLabels[0];
			String theTld = theLabels[1];
			...
			System.out
					.println("testTransferOfCompliantDomain: Generated unique domain label ("
							+ theLabel
							+ ") and domain name ("
							+ theDomainName
							+ ") for flow");
			...
			try {
				...
				theDnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testTransferOfCompliantDomain: DNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				theRnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testTransferOfCompliantDomain: RNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				System.out
						.println("testTransferOfCompliantDomain: Domain name "
								+ theDomainName
								+ " successfully created with DNVC (type = "
								+ theDnvc.getCodeType() + ", code = "
								+ theDnvc.getCodeValue()
								+ ") and RNVC (type = " + theRnvc.getCodeType()
								+ ", code = " + theRnvc.getCodeValue());
			}
			catch (Exception ex) {
			...
			try {
				...
				System.out
						.println("testTransferOfCompliantDomain: Step 1 - Domain name "
								+ theDomainName + " verification is compliant");
			...
			try {
				...
				System.out
						.println("testTransferOfCompliantDomain: Step 2 - Domain name "
								+ theDomainName
			...
			try {
				...
				System.out
						.println("testTransferOfCompliantDomain: Step 3 - Domain name "
								+ theDomainName
	...
	public void testTransferOfNonCompliantDomain() {
		...
		if (!this.sessionPool.hasSystemSessionPool(POOL_REGISTRY_2)) {
			System.out
					.println("testTransferOfNonCompliantDomain: "
							+ POOL_REGISTRY_2
							+ " session pool not defined for optional test, skipping test");
		...
		try {
			...
			System.out
					.println("testTransferOfNonCompliantDomain: Got sessions to the Registry (1 and 2) and to the Vsp");

			// --- Generate unique domain label and domain name

			String theDomainName = this.makeDomainName(
					"cvct-transfer-noncompliant-", ".com");
			System.out.println("theDomainName = " + theDomainName);
			String theLabels[] = theDomainName.split("\\.");
			String theLabel = theLabels[0];
			String theTld = theLabels[1];
			...
			System.out
					.println("testTransferOfNonCompliantDomain: Generated unique domain label ("
							+ theLabel
							+ ") and domain name ("
							+ theDomainName
							+ ") for flow");
			...
			try {
				...
				theDnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testTransferOfNonCompliantDomain: DNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				System.out
						.println("testTransferOfNonCompliantDomain: Domain name "
								+ theDomainName
								+ " successfully created with DNVC (type = "
								+ theDnvc.getCodeType()
								+ ", code = "
			...
			try {
				...
				// Assert the missing code is a real-name code
				Assert.assertTrue(theVerificationExt.getProfile()
						.getMissingCodes().get(0).getType().equals("real-name"));

				System.out
						.println("testTransferOfNonCompliantDomain: Step 1 - Domain name "
								+ theDomainName
			...
			try {
				...
				theRnvc = theSuccessResult.getSignedCode();
				System.out
						.println("testTransferOfNonCompliantDomain: Step 2 - RNVC successfully created, type = "
								+ theSuccessResult.getCodeType()
								+ ", code  = "
								+ theSuccessResult.getCode());
			}
			...
			try {
				...
				System.out
						.println("testTransferOfNonCompliantDomain: Step 3 - Domain name "
								+ theDomainName
	...
	public static Test suite() {
		return new ChinaVerificationCodeTstSetup(new TestSuite(
				ChinaVerificationCodeTst.class));
	}
	...
	private EPPDomainInfoResp findChangePoll(EPPSession aSession,
		...
		while ((theResponse.getResult().getCode() == EPPResult.SUCCESS_POLL_MSG)
			...
			System.out.println("findChangePoll: Poll Response = ["
					+ theResponse + "]\n\n");
			...
			if (theResponse instanceof EPPDomainTransferResp) {
				...
				System.out.println("findChangePoll: ID = "
					...
					&& (theResponse
							.hasExtension(EPPChangeData.class))) {
				System.out.println("findChangePoll: ID = "
						+ theResponse.getMsgQueue().getId()
						+ ", change poll message");
				...
				if (theChange.getReason().equals("Locale Data Policy")) {		
					System.out
					.println("findChangePoll: Received Locale Data Policy Change Poll Message");
				}

				if (aDomainPrefix == null
						|| theChangePollMsg.getName().startsWith(aDomainPrefix)) {
					System.out
							.println("findChangePoll: Found matching change poll message");
				}
				else {
					theChangePollMsg = null;
				}
	...
	private void invalidateSession(String aPool, EPPSession aSession) {
		try {
			...
			ex.printStackTrace();
			Assert.fail("invalidateSession(): Exception invalidating session in "
					+ aPool + ": " + ex);
		}
	}
